...
class DiscordClient Extends ConfigLoader
{
	...
	private function outbox($message)
	{
		...
		{
			case "MESSAGE_CREATE":	return $this->MESSAGE_CREATE($message);
			case "MESSAGE_REPLY":	return $this->MESSAGE_REPLY($message);
			case "MESSAGE_UPDATE":	return $this->MESSAGE_UPDATE($message);
			case "MESSAGE_DELETE":	return $this->MESSAGE_DELETE($message);
			case "GET_CHANNEL":	return $this->GET_CHANNEL($message);
		}
		return true;
	}
	...
	{
		$this->discord->getChannel($message["channel_id"])->getMessage($message["reply_to"])->then(function ($originalMessage) use ($message)
		{
	...
	{
		$this->discord->getChannel($message["channel_id"])->getMessage($message["id"])->then(function ($originalMessage) use ($message)
		{
	...
	{
		$this->discord->getChannel($message["channel_id"])->getMessage($message["id"])->then(function ($originalMessage)
		{
